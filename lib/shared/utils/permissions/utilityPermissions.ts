import { type Cleaner, type Role, RoleLevel, RoleStatus } from '@prisma/client';

import type { TransformedCleaner } from '@/types';
...
export const isTaskCleaner = ({
  ...
  cleaners: (Cleaner | TransformedCleaner)[];
  userId: string;
}) => {
  if (!cleaners) return false;
  return cleaners.some((cleaner) =>
    'userId' in cleaner ? cleaner.userId === userId : cleaner.id === userId
  );
};
