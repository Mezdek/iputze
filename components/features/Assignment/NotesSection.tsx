...
import { useCreateAssignmentNote, useDeleteAssignmentNote } from '@hooks';
import { datefy } from '@lib/shared';
import { useState } from 'react';

import type { AssignmentNoteWithAuthor, NotesSectionProps } from '@/types';
 ...
 * Displays assignment notes with author info
...
export function NotesSection({
  ...
  const handleDelete = async (note: AssignmentNoteWithAuthor) => {
    if (!confirm('Are you sure you want to delete this note?')) {
      return;
    }

    try {
      await deleteNote({ assignmentNoteId: note.id });
    } catch (error) {
      // Error handled by hook
    }
  };
        ...
        <Card className="shadow-none bg-default-50">
          <CardBody>
            <p className="text-center text-default-500 py-8">
              No notes have been added to this assignment yet.
            </p>
          </CardBody>
        </Card>
      ) : (
 ...
 */
export function NotesCompact({ notes }: { notes: AssignmentNoteWithAuthor[] }) {
  if (notes.length === 0) {
